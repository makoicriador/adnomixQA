 ...
 * Inactive, Active + Inactive), combining a filter with a search, and CSV
 * export. One authenticated session is shared across the suite (serial
...
test.describe('Service Providers > Suppliers Index', () => {
  ...
  test('filters suppliers by status = Active', async () => {
    ...
    await expect(suppliers.statusChip).toContainText('Active');
    await expect(suppliers.statusChip).not.toContainText('Inactive');
  ...
  test('filters suppliers by status = Inactive', async () => {
    ...
    await expect(suppliers.statusChip).toContainText('Inactive');
