...
/**
 * End-to-end coverage for Service Providers > Suppliers > Deactivate on
 * adxmanager.dev. Unlike Freight Forwarders/Warehouses, Suppliers' kebab
 * menu has no hard "Delete" action at all — only a "Deactivate" status
 * toggle (confirmed live) — so this suite exercises that toggle (and its
 * "Activate" reverse) instead of a delete flow. Each test creates its own
 * throwaway fixture so the real, pre-existing suppliers used by the Index
 * suite are never touched. One authenticated session is shared across the
 * suite (serial mode) since login is a real network round trip against a
 * live app.
 */
test.describe('Service Providers > Suppliers Deactivate', () => {
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    // Step 1-2: log in and navigate to Suppliers (login -> /home ->
    ...
    // the same sidebar path into Suppliers as the throwaway context above
  ...
  test('deactivates a supplier and removes it from the default (Active) index view', async () => {
    const fixture = buildFakeSupplierData({ companyName: `QA Automation Deactivate Fixture ${Date.now()}` });
    ...
    await suppliers.resetToCleanState();
    await suppliers.search(fixture.companyName);
    await expect(suppliers.rowsContaining(fixture.companyName).first()).toBeVisible();

    await suppliers.deactivateFor(fixture.companyName);

    // Verified live: redirects from the index straight to the record's own
    // detail page (no "/edit" suffix), where the success banner renders.
    await expect(page).toHaveURL(/\/service-providers\/suppliers\/[^/]+$/);
    await expect(suppliers.successAlert.first()).toContainText('Successfully deactivated the supplier.');

    // Confirm it disappeared from the default index view, which only shows
    // Active suppliers by default (confirmed live) — asserted via the "No
    // suppliers found" empty state rather than a `rowsContaining` row
    // count, since that empty state itself renders inside a `<tr>` echoing
    // the searched company name back, which would otherwise false-positive
    // as a "found" row.
    await suppliers.resetToCleanState();
    await suppliers.search(fixture.companyName);
  ...
  test('reactivating a deactivated supplier restores it to the default index view', async () => {
    const fixture = buildFakeSupplierData({ companyName: `QA Automation Reactivate Fixture ${Date.now()}` });
    await suppliers.createSupplier(fixture);
    await expect(suppliers.successAlert.first()).toContainText('Supplier created successfully.');
    ...
    await suppliers.activateFor(fixture.companyName);

    // Verified live: same detail-page redirect as Deactivate, with the
    // reverse success banner.
    await expect(page).toHaveURL(/\/service-providers\/suppliers\/[^/]+$/);
    await expect(suppliers.successAlert.first()).toContainText('Successfully activated the supplier.');
